            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {horarios.map((horario) => {
                  const fila = delDia.filter((a) => a.horarioTexto === horario)
                  const categoria = fila[0]?.categoria
                  return (
                    <tr key={horario} className="border-b border-base-100 last:border-0">
                      ...
                      <td className="px-5 py-2.5 whitespace-nowrap">{categoria && CATEGORIA_LABEL[categoria]}</td>
                      <td className="px-5 py-2.5">
                        {fila.map((a) => a.nombre).join(', ')}
                      </td>
                    </tr>
                  )
                })}
